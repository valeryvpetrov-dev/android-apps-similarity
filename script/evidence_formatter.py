...
def _build_deleted_code_direct_evidence(pair_row: dict) -> dict | None:
    if pair_row.get("deleted_code_direct_evidence_applied") is not True:
        return None
    evidence = make_evidence(
        source_stage="pairwise",
        signal_type="deleted_code_direct_evidence",
        magnitude=_clamp_unit(pair_row.get("deleted_code_direct_evidence_score")),
        ref=str(
            pair_row.get("deleted_code_direct_evidence_ref")
            or "R_deleted_code_method_delta"
        ),
    )
    evidence.update(
        {
            "evidence_role": str(
                pair_row.get("deleted_code_direct_evidence_role") or "evidence_only"
            ),
            "score_effect": str(
                pair_row.get("deleted_code_direct_score_effect") or "none"
            ),
            "score_included": bool(
            ...
            "right_method_count": pair_row.get(
                "deleted_code_direct_right_method_count"
            ),
            "common_method_id_count": pair_row.get(
                "deleted_code_direct_common_method_id_count"
            ),
            "left_only_method_count": pair_row.get(
                "deleted_code_direct_left_only_method_count"
            ),
            "right_only_method_count": pair_row.get(
                "deleted_code_direct_right_only_method_count"
            ),
            "deleted_fingerprint_count": pair_row.get(
                "deleted_code_direct_deleted_fingerprint_count"
            ),
            "left_only_method_ratio": pair_row.get(
                "deleted_code_direct_left_only_method_ratio"
            ),
            "top_method_prefixes": pair_row.get(
                "deleted_code_direct_top_method_prefixes"
            ),
            "method_sample": _coerce_string_list(
                pair_row.get("deleted_code_direct_method_sample"),
                limit=20,
            ),
        }
    )
...
def collect_evidence_from_pairwise(pair_row: dict) -> list[dict]:
    ...
    if pair_row.get("code_stats_containment_policy_applied") is True:
        evidence.append(
            make_evidence(
                source_stage="pairwise",
                signal_type="code_stats_containment",
                magnitude=_clamp_unit(pair_row.get("code_stats_containment_score")),
                ref="R_code_stats_containment",
            )
        )
        evidence.append(
            make_evidence(
                ...
                magnitude=_clamp_unit(
                    pair_row.get("code_stats_resource_corroboration_score")
                ),
                ref="resource_path_digest",
            )
        )
    ...
    if pair_row.get("code_stats_payload_resource_policy_applied") is True:
        evidence.append(
            make_evidence(
                source_stage="pairwise",
                signal_type="payload_resource_support",
                magnitude=_clamp_unit(pair_row.get("payload_resource_score")),
                ref="R_code_stats_payload_resource",
            )
        )
