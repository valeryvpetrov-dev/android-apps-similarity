...
def calculate_similarity_two_dots(
    ...
    sim = normalize_ged(ged, dots_1, dots_2)
...
def normalize_ged(ged: int, dots_1, dots_2) -> float:
    dots_1_size = dots_1.number_of_nodes() + dots_1.number_of_edges()
    dots_2_size = dots_2.number_of_nodes() + dots_2.number_of_edges()
    max_size = max(dots_1_size, dots_2_size)
    return 1 - ged / max_size


def normalize_ged_via_empty_graph(ged: int, dots_1, dots_2, ged_timeout_sec: int) -> float:
    ...
    return 1 - (ged / (edit_distance_1_0 + edit_distance_2_0))
