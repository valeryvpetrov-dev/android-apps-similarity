#!/usr/bin/env python3
"""Tests for large preserved-code-core pairwise scoring."""
...
class TestRepackCorePolicy(unittest.TestCase):
...
""".strip(),
    ...
    def test_large_preserved_core_promotes_with_bounded_code_delta(self) -> None:
        shared_code = {"fp:shared_{:04d}".format(index) for index in range(2200)}
        changed_code = {"fp:changed_{:04d}".format(index) for index in range(500)}
        left_resources = {
        ...
        result = self._run_one_pair(
            _resource_bundle(shared_code | changed_code, left_resources),
        ...
        self.assertTrue(result["code_stats_repack_core_policy_applied"])
        self.assertAlmostEqual(result["repack_core_score"], 2200 / 2700)
        self.assertEqual(result["repack_core_method_count"], 2200)
        self.assertAlmostEqual(result["repack_core_added_code_delta"], 500 / 2700)
        self.assertAlmostEqual(result["repack_core_resource_support_score"], 1 / 3)
        self.assertEqual(
            result["similarity_score_source"],
            "code_stats_repack_core_with_bounded_code_delta",
        )
        self.assertAlmostEqual(result["similarity_score"], 2200 / 2700)
        ...
        self.assertAlmostEqual(
            evidence_refs[("large_preserved_code_core", "R_code_stats_repack_core")],
            2200 / 2700,
        )
